...
VISTA 1
                        ...
                        select  DOCUMENTOUSUARIO.FIRSTNAME ||' '||DOCUMENTOUSUARIO.LASTNAME AS NOMBRE, SUM(APUESTA.TOTAL_PROFIT) AS VALOR_ACUMULADO 
                        FROM MATCH_ PARTIDO
                        INNER JOIN QUOTA_MATCH CUOTAPARTIDO ON CUOTAPARTIDO.FK_MATCH = PARTIDO.ID_MATCH
                        INNER JOIN BET_DETAIL DETALLEAPUESTA ON DETALLEAPUESTA.FK_QUOTA_MATCH = CUOTAPARTIDO.ID_QUOTA_MATCH
                        INNER JOIN BET APUESTA ON APUESTA.ID_BET = DETALLEAPUESTA.FK_BET
                        INNER JOIN DATAUSER DATOSUSUARIOS ON DATOSUSUARIOS.ID_USER = APUESTA.FK_DATAUSER
                        INNER JOIN DOCUMENT_USER DOCUMENTOUSUARIO ON DOCUMENTOUSUARIO.ID_DOCUMENT = DATOSUSUARIOS.FK_DOCUMENT
                        WHERE PARTIDO.MATCH_DATE between trunc(sysdate, 'DAY') and trunc(sysdate, 'DAY')+6 
                        AND APUESTA.STATUS ='GANADA'
                        GROUP BY DOCUMENTOUSUARIO.FIRSTNAME ||' '||DOCUMENTOUSUARIO.LASTNAME
                        ORDER BY VALOR_ACUMULADO DESC;   
	  
procediomiento 2(funcion 1)
CREATE OR REPLACE FUNCTION FNC_USER_LOGUIN 
(  ID IN NUMBER ) 
RETURN VARCHAR2 AS 
  LOGUEADO AS NUMBER;
BEGIN
  BEGIN <<CONSULTA_USUARIO_LOGUEADO>>
  SELECT COUNT(*) INTO LOGUEADO FROM DATAUSER
  WHERE LOGIN_SESSION = 'Y' AND ID_USER = ID; 
  END
  IF LOGUEADO >0 THEN
  RETURN 'TRUE';
  END IF;
